...
def save(ckpt_dir: str, state, postfix=None) -> None:
    postfix = f".{postfix}" if postfix is not None else ''
    with open(os.path.join(ckpt_dir, f"arrays{postfix}.npy"), "wb") as f:
        for x in jax.tree_util.tree_leaves(state):
            np.save(f, x, allow_pickle=False)

    tree_struct = jax.tree_util.tree_map(lambda t: 0, state)
    with open(os.path.join(ckpt_dir, f"tree{postfix}.pkl"), "wb") as f:
        pickle.dump(tree_struct, f)
    if postfix != '':
        save(ckpt_dir, state, postfix=None)


def restore(ckpt_dir, postfix=None):
    postfix = f"{postfix}" if postfix is not None else ''
    with open(os.path.join(ckpt_dir, f"tree{postfix}.pkl"), "rb") as f:
        tree_struct = pickle.load(f)

    leaves, treedef = jax.tree_util.tree_flatten(tree_struct)
    with open(os.path.join(ckpt_dir, f"arrays{postfix}.npy"), "rb") as f:
