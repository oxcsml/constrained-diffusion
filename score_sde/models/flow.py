...
from score_sde.product import ProductSDE, ProductRSDE
...
from score_sde.sampling import get_discrete_pc_sampler as get_pc_sampler
from score_sde.product import get_product_discrete_pc_sampler as get_product_pc_sampler
...
def get_product_riemannian_div_fn(func, hutchinson_type: str = "None", manifolds=None):
    """divergence of the drift function.
    if M is submersion with euclidean ambient metric: div = div_E
    else (in a char) div f = 1/sqrt(g) \sum_i \partial_i(sqrt(g) f_i)
    """
    # TODO: we need to add back the sqrt g method
    drift_fn = lambda y, t, context: jnp.hstack(func(y, t, context))
    div_fn = get_div_fn(drift_fn, hutchinson_type)
    return lambda y, t, context, eps: div_fn(jnp.hstack(y), t, context, eps)
...
class SDEPushForward(PushForward):
    ...
    def get_sampler(
        ...
        if self.diffeq == "ode":  # via probability flow
            ...
            def sample(rng, shape, context, z=None, N=None, t0=None, tf=None):
                z = self.base.sample(rng, shape) if z is None else z
                ...
                y, t = sampler(rng, z, N=N, t0=t0, tf=tf)
                x = self.transform(y) if transform else y
                return x, t
...
class ProductCNF:
    ...
    def get_forward(self, model_w_dicts, train, augmented=False, **kwargs):
        ...
        def forward(data, context=None, t0=None, tf=None, rng=None, reverse=False):
            ...
            dims = np.cumsum([0] + [component.shape[1] for component in data])
            epsilons = div_noise(rng, jnp.hstack(data).shape, hutchinson_type)
            t0 = self.t0 if t0 is None else t0
            tf = self.tf if tf is None else tf
            eps = kwargs.get("eps", 1e-3)
            ts = jnp.array([t0 + eps, tf])
            ...
            if augmented:  # Also solving for the change in log-likelihood
                ...
                def ode_func(
                    ...
                    # sample = y[:, :-1]
                    sample = [y[:, s:e] for (s, e) in zip(dims[:-1], dims[1:])]
                    vec_t = jnp.ones((n_data,)) * t
                    drift_fn = self.get_drift_fn(model, params, states)
                    drift = drift_fn(sample, vec_t, context)
                    div_fn = get_product_riemannian_div_fn(
                        drift_fn, hutchinson_type, self.manifolds
                    )
                    logp_grad = div_fn(sample, vec_t, context, epsilons).reshape(-1, 1)
                    return jnp.concatenate([drift, logp_grad], axis=1)

                data = jnp.hstack(data).reshape(n_data, -1)
                init = jnp.concatenate([data, np.zeros((n_data, 1))], axis=1)
                ode_func = ReverseAugWrapper(ode_func, tf) if reverse else ode_func
                y, nfe = odeint(
                    ode_func, init, ts, context, params, states, **ode_kwargs
                )
                ...
                z = y[-1].reshape(n_data, -1)# .reshape(shape)
                return z, nfe

        return forward


class ProductSDEPushForward(PushForward):
    def __init__(self, product: ProductSDE, bases, diffeq="sde", transform=Id):
        ...
        flow = ProductCNF(
            t0=self.product.t0,
            tf=self.product.tf,
            get_drift_fn=partial(get_sde_drift_from_fn, self.product),
            manifolds=[sde.manifold for sde in product.sdes]
        )
        super(ProductSDEPushForward, self).__init__(flow, bases, transform)
    ...
    def get_sampler(
        ...
        if self.diffeq == "ode":  # via probability flow
            ...
            def sample(rng, shape, context, zs=None, N=None, t0=None, tf=None):
                z = [base.sample(rng, shape) for base in self.bases] if zs is None else zs
                score_fn = self.product.reparametrise_score_fn(*model_w_dicts)
                score_fn = partial(score_fn, context=context)
                product = ProductRSDE(self.product, score_fn) if reverse else self.product
                sampler = get_product_pc_sampler(product, **kwargs)
                sampler = jax.jit(sampler)
                y, t = sampler(rng, z, N=N, t0=t0, tf=tf)
                x = self.transform(y) if transform else y
                return x, t
