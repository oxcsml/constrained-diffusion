...
class SphericalDataset(CSVDataset):
    def __init__(self, file, extrinsic=False, delimiter=",", skip_header=1, batch_size=256):
        super().__init__(file, delimiter=delimiter, skip_header=skip_header, batch_size=batch_size)
...
class USForestFire(SphericalDataset):
    def __init__(self, data_dir="data", batch_size=256, **kwargs):
        super().__init__(os.path.join(data_dir, "us_forest_fires.csv"), batch_size=batch_size)