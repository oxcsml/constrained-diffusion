...
class AmbientTorusGenerator(VectorFieldGenerator):
    def __init__(self, architecture, embedding, output_shape, manifolds):
        super().__init__(architecture, embedding, output_shape, manifolds)

        self.rot_mat = jnp.array([[0, -1], [1, 0]])

    @staticmethod
    def output_shape(manifolds):
        return [manifold.dim for manifold in manifolds]

    def _generators(self, x):
        return self.ambient_generators(x[0]), self.torus_generators(x[1])

    def ambient_generators(self, x):
        return None

    def torus_generators(self, x):
        return (self.rot_mat @ x.reshape((*x.shape[:-1], -1, 2))[..., None])[
            ..., 0
        ].reshape(*x.shape[:-1], -1)
    ...
    def torus_generators(self, x):
        return (
            self.rot_mat
            @ x.reshape((*x.shape[:-1], self.manifold[1].dim, 2))[..., None]
        )[..., 0]

    def __call__(self, x, t):
        if not isinstance(x, list):
            x = [x[:, : self.manifold[0].dim], x[:, self.manifold[0].dim :]]
        weights_fn, fields_fn = self.decomposition
        weights = weights_fn(x, t)
        fields = fields_fn(x)

        ambient_score = weights[0]
        torus_score = (fields[1] * weights[1][..., None]).reshape(
            (*x[1].shape[:-1], self.manifold[1].dim * 2)
        )

        return ambient_score, torus_score
